...
/* Check value if it matches expected an throw exception if not */
template<typename TYPE>
inline void checkValue(const QString& message, TYPE value, TYPE expected)
{
  if(expected != value)
    throw new atools::Exception(QString("GribReader: Error reading grib file: "
                                        "%1: value %2 not equal to expected value %3").
                                arg(message).arg(value).arg(expected));
}

/* Check value if it matches one of the expected an throw exception if not */
template<typename TYPE>
void checkValue(const QString& message, TYPE value, const QVector<TYPE>& expected)
{
  if(!expected.contains(value))
  {
    ...
    throw new atools::Exception(QString("GribReader: Error reading grib file: %1: value %2 not in expected range %3").
                                arg(message).arg(value).arg(expectedStr.join(", ")));
  }
}
...
void GribReader::readFile(const QString& filename)
{
  ...
  while(true)
  {
    ...
    for(long n = 0; n < numfields; n++)
    {
      ...
      checkValue("Datetime is not valid", dataset.datetime.isValid(), true);
      ...
      checkValue("Grid definition", gribField->griddef, g2int(0));
      ...
      checkValue("Grid Definition Template Number", gribField->igdtnum, g2int(0));
      ...
      checkValue("shape of earth", gribField->igdtmpl[0], g2int(6));
      checkValue("radius scale factor", gribField->igdtmpl[1], g2int(0));
      checkValue("scale value", gribField->igdtmpl[2], g2int(0));
      checkValue("scale factor of major axis", gribField->igdtmpl[3], g2int(0));
      checkValue("scale value of major axis", gribField->igdtmpl[4], g2int(0));
      checkValue("scale factor of minor axis", gribField->igdtmpl[5], g2int(0));
      checkValue("scale value of minor axis", gribField->igdtmpl[6], g2int(0));
      checkValue("Ni", gribField->igdtmpl[7], g2int(360));
      checkValue("Nj", gribField->igdtmpl[8], g2int(181));
      checkValue("Basic angle", gribField->igdtmpl[9], g2int(0));
      checkValue("resolution component flags", gribField->igdtmpl[13], g2int(48));
      checkValue("scanning mode flags", gribField->igdtmpl[18], g2int(0));

      // checkValue("i increment", gfld->igdtmpl[16], g2int(1));
      // checkValue("j increment", gfld->igdtmpl[17], g2int(1));
      ...
      checkValue("Parameter category", gribField->ipdtmpl[0], g2int(2));
      checkValue("Parameter number", gribField->ipdtmpl[1], {g2int(2), g2int(3)});
      if(gribField->ipdtmpl[1] == 2)
        dataset.parameterType = U_WIND;
      else if(gribField->ipdtmpl[1] == 3)
        dataset.parameterType = V_WIND;

      checkValue("Time range", gribField->ipdtmpl[7], g2int(1));
      checkValue("Surface type", gribField->ipdtmpl[9], {g2int(100), g2int(103)});
      ...
      checkValue("Second surface scale factor", gribField->ipdtmpl[13], g2int(0));
      checkValue("Second surface value", gribField->ipdtmpl[14], g2int(0));
      ...
      checkValue("Unpacked", gribField->unpacked, g2int(1));
      ...
      // "1" in gfld->bmap.
      checkValue("Unpacked", gribField->expanded, g2int(1));
      ...
      if(verbose)
      {
        qDebug() << "dataset.altFeetRounded" << dataset.altFeetRounded
                 << "dataset.parameterType" << dataset.parameterType;
...
void GribReader::readData(const QByteArray& data)
{
  ...
  {
    file.remove();
    throw new atools::Exception(QString("Cannot create temporary file %1").arg(file.fileName()));
  }
  file.remove();
}
