...
void ApronWriter::writeObject(const QPair<const bgl::Apron *, const bgl::Apron2 *> *type)
{
  ...
  bind(":vertices", toWkt(type->first->getVertices()));
  bind(":vertices2", toWkt(type->second->getVertices()));
  bind(":triangles", toString(type->second->getTriangles()));

  executeStatement();
}

QString ApronWriter::toWkt(const QList<atools::fs::bgl::BglPosition>& vertices)
{
  // TODO use real WKT
  QString retval;
  for(const bgl::BglPosition& pos : vertices)
  {
    if(!retval.isEmpty())
      retval += ",";
    retval.append(QString::number(pos.getLonX())).append(",");
    retval.append(QString::number(pos.getLatY()));
  }
  return retval;
}
